...
void MewParamTable::reindexing_table_cells() {
...
MewParamConfigCellStr::MewParamConfigCellStr(MewParamTable& parent_table) : MewParamConfigCell() {
...
void MewParamConfigCellStr::load_data(Wt::Json::Object data) {
...
Wt::Json::Object MewParamConfigCellStr::get_obj() {
    Wt::Json::Object obj;
...
MewParamConfigCellNum::MewParamConfigCellNum(MewParamTable& parent_table) : MewParamConfigCell() {
    class_type = addWidget(std::make_unique<MewCellString>("class", ""));
    tag = addWidget(std::make_unique<MewCellString>("tag", ""));
    property = addWidget(std::make_unique<MewCellString>("property", ""));
...
void MewParamConfigCellNum::load_data(Wt::Json::Object data) {
    class_type->set_data(data["class"]);
    tag->set_data(data["tag"]);
    property->set_data(data["property"]);
    val_file->set_data(data["val_file"]);
}

Wt::Json::Object MewParamConfigCellNum::get_obj() {
    Wt::Json::Object obj;
    obj["class"] = Wt::Json::Value(class_type->get_data().second);
    obj["tag"] = Wt::Json::Value(tag->get_data().second);
    obj["property"] = Wt::Json::Value(property->get_data().second);
    obj["val_file"] = Wt::Json::Value(val_file->get_data().second);
    return obj;
}

MewSimConfigPannel::MewSimConfigPannel(Wt::WContainerWidget* parent) : Wt::WContainerWidget() {
...
void MewSimConfigPannel::load_from_file(std::string filePath) {
    ...
    for (const auto& param_obj : param_list_json) {
        if (param_obj.type() == Wt::Json::Type::Object) {
            ...
            if (!param_obj_con.isNull("string_file")) {
            ...
            } else if (!param_obj_con.isNull("val_file")) {
...
void MewSimConfigPannel::dump_config() {
    ...
    for (int i=0; i<paramTable->rowCount(); i++) {
        if(dynamic_cast<MewParamConfigCellStr*>(paramTable->elementAt(i, 0)->widget(0)) != nullptr) {
            ...
            paramList.push_back(((MewParamConfigCellNum*)(paramTable->elementAt(i, 0)->widget(0)))->get_obj());
        }
    }
